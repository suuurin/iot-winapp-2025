...
- 날짜별 일정 및 일기 관리 (날짜 선택 시 해당 날짜의 일기, 할일 불러올 수 있음)
- 할 일 체크리스트 추가
- 선택 항목 수정 및 삭제 (항목 선택 시 텍스트 불러오고 수정, 삭제 가능)
- 한 줄 일기 입력/변경 바로 저장

<img src="./image/pr02.png" width="600">

- 저장 / 불러오기
    - 프로그램 종료 시 날짜별 모든 데이터 프로젝트 위치에 data.txt 파일로 저장
    - [O][X] 체크 상태 함께 저장해 종료 후에도 유지
    - 시작 시 데이터 자동으로 불러오기
